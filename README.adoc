...
.現在の状態（の一部）がそのまま出力となる順序回路（ムーアマシン出力直結）
...
<<fig:moore_machine_alpha>> の構成は、特に一般的な名称は無いが、本文書ではムーアマシンと区別するため、出力直結ムーアマシンと呼ぶことにする。出力直結ムーアマシンは、本文書独自の呼び名であるので、他では使用しないよう注意されたい。
...
[[fig:moore_machine_synthesized_cirvuit]]
.エッジ検出回路の合成結果（ムーアマシン）
image:moore_machine_synthesized_cirvuit.svg[] 
